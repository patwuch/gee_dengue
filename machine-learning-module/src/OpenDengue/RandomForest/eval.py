...
try:
    ...
    # --- Standalone CLI mode ---
    _p = argparse.ArgumentParser(description="Evaluate Random Forest — OpenDengue (standalone)")
    ...
    _p.add_argument("--model",   default=None,  help="Override model .joblib path")
    _args = _p.parse_args()
    ...
    model_path  = Path(_args.model) if _args.model else paths["models_dir"] / "rf" / study / f"{region}.joblib"
    results_csv = paths["tables_dir"] / study / f"{region}_rf_results.csv"
    results_csv.parent.mkdir(parents=True, exist_ok=True)
